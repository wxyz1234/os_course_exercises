...
### 6.4	页表概述
 1. 每个页表项有些什么内容？有哪些标志位？它们起什么作用？
     - 每个页表项有对应帧号，标志位有存在位、修改位、引用位。存在位是指这个逻辑页号是否有对应的物理帧。修改位是指这个逻辑页号对应的物理帧是否被修改了。引用位是指过去一段时间里是否有过对这个对应帧的引用。
     
 1. 页表大小受哪些因素影响？
     - 页表大小主要受页大小、地址空间、进程数目和页表级数影响。

### 6.5	快表和多级页表
 1. 快表（TLB）与高速缓存（cache）有什么不同？
     - 快表是一个专门用来处理虚拟地址转换的高速缓存。
     
 1. 为什么快表中查找物理地址的速度非常快？它是如何实现的？为什么它的的容量很小？
     - 因为快表是并行访问，物理硬件实现，容量小的原因是要节约成本。
     
 1. 什么是多级页表？多级页表中的地址转换流程是什么？多级页表有什么好处和麻烦？
     - 多级页表通过间接引用将页号分成若干级的页表。多级页表的地址转换流程类似页表，从上一级级页号查询下一级级页表的基址然后再通过下一级页号继续查询，直到找到帧号再通过页内偏移得到最终的物理地址。
     - 好处是可以减小页表所占用的空间，缺点是访问次数更多，速度更慢。


### 6.6	反置页表
 1. 页寄存器机制的地址转换流程是什么？
     - 通过逻辑地址的哈希找出所有哈希值一致的页寄存器，然后遍历这些页寄存器判断其中是否有我们想要的物理帧（逻辑地址符合要求）。
     
 1. 反置页表机制的地址转换流程是什么？
     - 大部分转换流程都是一样的，区别只是做哈希的时候同时把进程号也考虑进去。
     - 通过逻辑地址和进程号的哈希找出所有哈希值一致的页寄存器，然后遍历这些页寄存器判断其中是否有我们想要的物理帧（逻辑地址和进程号符合要求）。
     
 1. 反置页表项有些什么内容？
     - PID（进程号）、逻辑页号、标志位（使用位：当前物理帧是否被进程占用。保护位：约定这一页的访问方式，比如可读可写。）

### 6.7	段页式存储管理
 1. 段页式存储管理机制的地址转换流程是什么？这种做法有什么好处和麻烦？
     - 先通过进程的段基址和段号找到相应的段表项，然后通过段表项和页号找到相应的页表项，即物理页帧号，再加上页内偏移即为所求。
     - 段式存储在内存保护上有优势，页式存储在内存利用和优化转移到后备储存方式上有优势，这么做可以把两边的好处相结合，缺点是运行成本变高了。
